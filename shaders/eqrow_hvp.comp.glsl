...
layout(set = 0, binding = 11, std430) readonly buffer VB { float vvec[]; };
layout(set = 0, binding = 12, std430)          buffer OB { float outv[]; };
...
void main() {
    ...
    /* Jv = sum over combos/channels of A[ch]*v[prim+ch] */
    float Jv = 0.0;
    for (int i=0;i<meta.num_combos;i++){ combo(i,ii,fr,pob,prim,W);
        float A[NCH];
        ...
        for (int e=q0; e<q1; e++) {
            ...
            int s1=(pk>>20)&15, c1=(pk>>16)&15, s2=(pk>>12)&15, c2=(pk>>8)&15;
            A[c1] += v * fld[s2][c2] * W[s1];
            A[c2] += v * fld[s1][c1] * W[s2];
        }
        [[unroll]] for(int ch=0;ch<NCH;ch++) Jv += A[ch]*vvec[prim+ch];
    }
    ...
    for (int i=0;i<meta.num_combos;i++){ combo(i,ii,fr,pob,prim,W);
        ...
        [[unroll]] for(int ch=0;ch<NCH;ch++)
            if (A[ch] != 0.0) atomicAdd(outv[prim+ch], A[ch]*y);
    }
}
