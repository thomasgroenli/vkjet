/* vkflow — CG damping term: y[i] += a · d[i] · x[i]  (Ap += μ·diag∘p). */
#version 460
...
layout(set=0,binding=2,std430)          buffer Y { float y[]; };
void main(){
    uint stride = gl_NumWorkGroups.x*gl_WorkGroupSize.x;
    for(uint i=gl_GlobalInvocationID.x; i<uint(meta.n); i+=stride) y[i]+=meta.a*d[i]*x[i];
}
